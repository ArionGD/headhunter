...
def record_user_activity(request, action, description, target_lead=None):
    """
    Records an activity log and updates ONLY the active user's last_seen & last_action metadata.
    Mirrors updates to Turso Cloud.
    """
    ...
    # 2. Update ONLY the active user's UserMetaTracker
    try:
        now = timezone.now()
        meta, created = UserMetaTracker.objects.get_or_create(
            user_id=user_id,
            defaults={
                'role': role,
...
def update_user_last_seen(request):
    """Quick update of last seen for active user session only"""
